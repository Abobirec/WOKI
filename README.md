...
## Возможности (как в Cursor)

- ⚡ **Стриминг** — ответ печатается вживую, есть кнопка «⏹ Стоп».
- ✅ **Автопроверка** — после правок агент сам запускает тесты/линтер и чинит
  ошибки, пока не станет чисто.
- 🧠 **Память проекта** — диалог сохраняется отдельно для каждого проекта и
  восстанавливается после перезапуска; ключевые факты агент пишет в
  `.woki/memory.md` и помнит их в следующих сессиях.
- 🔐 **Хранилище ключей проекта** — даёшь именованные API-ключи (⚙ → «API-ключи
  этого проекта»). Они шифруются отдельно для каждого проекта, а агент вписывает
  их в `.env` через `save_secret_to_env`, **не видя самого значения**.
- 📄 **Контекст активного файла** — агент может прочитать открытый файл и
  выделенный фрагмент (`read_active_editor`).
